...
class ObjectDetectionRNPrimitive(PrimitiveBase[Inputs, Outputs, Params, Hyperparams]):
    ...
    def _annotation_row_to_groundtruth_list(self, annotations):
        """ 
        Take annotation DataFrame and extract out the image names and ground truth coordinates in the
        8-coordinate format required by the metrics evaluator.

        Parameters
        ----------
        annotations     : Annotation data set generated in _set_training_data().

        Returns
        -------
        image_box_list  : Extracted image names and ground truth coordinates in a list.
        """

        path = os.path.basename(annotations['path'])
        coords = annotations[['x1A', 'y1A', 'x1B', 'y2A', 'x2A', 'y2B', 'x2B', 'y1B']]
        image_box_list = [path]
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> CallResult[Outputs]:
        ...
        # Assemble fourth output column ('confidence_score')
        ...
        ## Generate ground truth bounding boxes (8-coordinate format, list)
        # eight_coordinate_annotations = self.annotations.reindex(['path', 'x1', 'y1', 'x1', 'y2', 'x2', 'y2', 'x2', 'y1'], axis = 1)
        # eight_coordinate_annotations.columns = ['path', 'x1A', 'y1A', 'x1B', 'y2A', 'x2A', 'y2B', 'x2B', 'y1B']
        # y_true = eight_coordinate_annotations.apply(_annotation_row_to_groundtruth_list, axis = 1).to_list()
